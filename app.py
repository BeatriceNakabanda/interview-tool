...
if not st.session_state["setup_complete"]:
    ...
    st.subheader('Company and Position', divider = 'rainbow')
     ...
     # Initialize session state for company and position information and setting default value
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    # Initialize session state if not checks if teh message list is empty
    if len(st.session_state["messages"]) == 0:
        ...
        st.session_state.messages = [{"role": "system", 
                                      "content": f"""
                                            ...
                                            - Ask exactly 4 questions total.
                                            - After the 4th answer, say: 'Thank you. The interview is complete.'

                                            Candidate experience: {st.session_state['experience']}
        ...
        messages= [
            {
                ...
                Give only feedback do no ask any additional questions.
                """
            },
            {
                "role": "user", "content": f"This is the interview you need to evaluate. Keep in mind that you are only a tool. And you shouldn't engage in any conversation: {converstation_history}"
            }
        ]
    )
